...
<x-layouts::app title="Insurance Expiry Report">
    ...
    <div class="mb-5 flex items-center justify-between flex-wrap gap-3">
        <div>
            ...
            <flux:text class="text-zinc-400 mt-1">Insurance policy expiry dates.</flux:text>
        </div>
            ...
            <tbody class="divide-y divide-zinc-800">
                @forelse ($records as $policy)
                    ...
                    <tr class="hover:bg-accent/5">
                        ...
                        <td class="px-4 py-2.5"><a href="{{ route('assets.show', [$policy->asset, 'tab' => 'insurance']) }}" class="font-mono text-xs font-semibold text-accent">{{ $policy->asset?->asset_code }}</a></td>
                        <td class="px-4 py-2.5 font-medium text-zinc-100">{{ $policy->asset?->asset_name }}</td>
                        <td class="px-4 py-2.5 text-zinc-400">{{ $policy->asset?->category?->name ?: '—' }}</td>
                        <td class="px-4 py-2.5 text-zinc-400">{{ $policy->insurer_name ?: '—' }}</td>
                        <td class="px-4 py-2.5 font-mono text-xs text-zinc-300">{{ $policy->policy_number ?: '—' }}</td>
                        <td class="px-4 py-2.5 text-zinc-400">{{ $policy->policy_type ?: '—' }}</td>
                        <td class="px-4 py-2.5 text-zinc-400">{{ $policy->policy_date_from?->format('d M Y') ?: '—' }}</td>
                        ...
                        <td class="px-4 py-2.5 text-right text-zinc-300">{{ $policy->premium_amount ? '₹ ' . number_format($policy->premium_amount, 2) : '—' }}</td>
                    </tr>
                @empty
