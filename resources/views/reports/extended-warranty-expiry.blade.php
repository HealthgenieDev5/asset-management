...
@endphp
<x-layouts::app title="Extended Warranty Expiry Report">
    @include('partials.flash')

    <div class="mb-5 flex items-center justify-between flex-wrap gap-3">
        <div>
            <flux:heading size="xl" class="font-extrabold">Extended Warranty <span class="text-accent">Expiry</span></flux:heading>
            <flux:text class="text-zinc-500 dark:text-zinc-400 mt-1">Extended / third-party warranty expiry dates.</flux:text>
        </div>
    ...
    @include('reports._filters', ['showExpiry' => true, 'expiryOptions' => $expiryFilterOptions,
        'exportUrl' => route('reports.extended-warranty-expiry.export', request()->query())])
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                @forelse ($records as $ew)
                    ...
                    <tr class="hover:bg-accent/5">
                        ...
                        <td class="px-4 py-2.5">@if($ew->asset)<a href="{{ route('assets.show', [$ew->asset, 'tab' => 'ext-warranty']) }}" class="font-mono text-xs font-semibold text-accent">{{ $ew->asset->asset_code }}</a>@else<span class="font-mono text-xs text-zinc-400">—</span>@endif</td>
                        <td class="px-4 py-2.5 font-medium text-zinc-800 dark:text-zinc-100">{{ $ew->asset?->asset_name }}</td>
                        <td class="px-4 py-2.5 text-zinc-500 dark:text-zinc-400">{{ $ew->asset?->category?->name ?: '—' }}</td>
                        <td class="px-4 py-2.5 text-zinc-500 dark:text-zinc-400">{{ $ew->extended_warranty_vendor ?: '—' }}</td>
                        <td class="px-4 py-2.5 font-mono text-xs text-zinc-600 dark:text-zinc-300">{{ $ew->extended_warranty_bill_no ?: '—' }}</td>
                        <td class="px-4 py-2.5 text-zinc-500 dark:text-zinc-400">{{ $ew->extended_warranty_date_from?->format('d M Y') ?: '—' }}</td>
                        ...
                        <td class="px-4 py-2.5 text-right text-zinc-600 dark:text-zinc-300">{{ $ew->extended_warranty_amount ? '₹ ' . number_format($ew->extended_warranty_amount, 2) : '—' }}</td>
                    </tr>
                @empty
