...
@endphp
<x-layouts::app title="Road Tax Expiry Report">
    @include('partials.flash')

    <div class="mb-5 flex items-center justify-between flex-wrap gap-3">
        <div>
            <flux:heading size="xl" class="font-extrabold">Road Tax <span class="text-accent">Expiry</span></flux:heading>
            <flux:text class="text-zinc-500 dark:text-zinc-400 mt-1">Vehicle road tax expiry dates.</flux:text>
        </div>
    ...
    @include('reports._filters', ['showExpiry' => true, 'expiryOptions' => $expiryFilterOptions,
        'exportUrl' => route('reports.road-tax-expiry.export', request()->query())])
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-zinc-200 text-left text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:border-zinc-800">
                    ...
                    <th class="px-4 py-3">Road Tax Expiry</th><th class="px-4 py-3">Status</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                @forelse ($assets as $asset)
                    @php
                        $days = (int) now()->startOfDay()->diffInDays($asset->road_tax_expiry_date->startOfDay(), false);
