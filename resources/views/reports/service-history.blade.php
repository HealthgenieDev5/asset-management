<x-layouts::app title="Service History Report">
    ...
    <div class="mb-5 flex items-center justify-between flex-wrap gap-3">
        <div>
            ...
            <flux:text class="text-zinc-500 dark:text-zinc-400 mt-1">Complete service, maintenance, and inspection history.</flux:text>
        </div>
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                @forelse ($records as $svc)
                    ...
                    <tr class="hover:bg-accent/5">
                        ...
                        <td class="px-4 py-2.5 text-zinc-500 dark:text-zinc-400">{{ $svc->technician_name ?: '—' }}</td>
                        ...
                        <td class="px-4 py-2.5 text-right text-zinc-600 dark:text-zinc-300">{{ $partsCost > 0 ? '₹ ' . number_format($partsCost, 2) : '—' }}</td>
                        <td class="px-4 py-2.5 text-right font-semibold text-zinc-800 dark:text-zinc-100">{{ $total > 0 ? '₹ ' . number_format($total, 2) : '—' }}</td>
                    </tr>
                @empty
                    <tr><td colspan="13" class="px-4 py-12 text-center text-zinc-500">No records found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
