<x-layouts::app title="Maintenance Cost Report">
    ...
    <div class="mb-5 flex items-center justify-between flex-wrap gap-3">
        <div>
            ...
            <flux:text class="text-zinc-400 mt-1">Total cost analysis across all service and parts records.</flux:text>
        </div>
        @if ($grandTotal > 0)
            ...
            <tbody class="divide-y divide-zinc-800">
                @forelse ($records as $svc)
                    ...
                    <tr class="hover:bg-accent/5">
                        ...
                        <td class="px-4 py-2.5 text-right text-zinc-300">{{ $pCost > 0 ? '₹ ' . number_format($pCost, 2) : '—' }}</td>
                        <td class="px-4 py-2.5 text-right font-semibold text-zinc-100">{{ $tot > 0 ? '₹ ' . number_format($tot, 2) : '—' }}</td>
                    </tr>
                @empty
            ...
            @if ($grandTotal > 0 && $records->isNotEmpty())
                <tfoot>
                    <tr class="border-t border-zinc-700 bg-zinc-800/50">
                        <td colspan="7" class="px-4 py-3 text-right text-xs font-semibold text-zinc-400 uppercase">Page Total</td>
                        <td class="px-4 py-3 text-right font-bold text-zinc-200">₹ {{ number_format($totalService, 2) }}</td>
                        <td class="px-4 py-3 text-right font-bold text-zinc-200">₹ {{ number_format($totalParts, 2) }}</td>
