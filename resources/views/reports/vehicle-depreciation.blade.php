<x-layouts::app title="Vehicle Depreciation Report">
    ...
    <div class="mb-5 flex items-center justify-between flex-wrap gap-3">
        <div>
            ...
            <flux:text class="text-zinc-400 mt-1">Original book value and current book value for vehicles.</flux:text>
        </div>
            ...
            <tbody class="divide-y divide-zinc-800">
                @forelse ($assets as $asset)
                    <tr class="hover:bg-accent/5">
                        ...
                        <td class="px-4 py-2.5 text-zinc-400">{{ $asset->purchase_date?->format('d M Y') ?: '—' }}</td>
                        <td class="px-4 py-2.5 text-right text-zinc-200">{{ $asset->vehicle_obv ? number_format($asset->vehicle_obv, 2) : '—' }}</td>
                        <td class="px-4 py-2.5 text-right text-zinc-400">{{ $asset->vehicle_depreciation_percent ? $asset->vehicle_depreciation_percent . '%' : '—' }}</td>
                        <td class="px-4 py-2.5 text-right font-semibold text-zinc-100">{{ $asset->vehicle_depreciation_book_value ? number_format($asset->vehicle_depreciation_book_value, 2) : '—' }}</td>
                ...
                @empty
                    <tr><td colspan="10" class="px-4 py-12 text-center text-zinc-500">No vehicle depreciation records found.</td></tr>
                @endforelse
            </tbody>
            @if ($assets->isNotEmpty() && ($totalObv > 0 || $totalBook > 0))
                <tfoot>
                    <tr class="border-t border-zinc-700 bg-zinc-800/50">
                        <td colspan="6" class="px-4 py-3 text-right text-xs font-semibold text-zinc-400 uppercase">Page Total</td>
                        <td class="px-4 py-3 text-right font-bold text-zinc-200">{{ number_format($totalObv, 2) }}</td>
                        <td></td>
