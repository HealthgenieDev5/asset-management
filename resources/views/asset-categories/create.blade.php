        ...
        <div class="mb-6 flex items-center gap-3">
            ...
            <div>
                <flux:heading size="xl" class="font-extrabold">
                    Add <span class="text-accent">Category</span>
                </flux:heading>
                <flux:text class="mt-0.5 text-zinc-400">Create a new asset category with a unique 2-character code.</flux:text>
            </div>
        </div>

        <div class="max-w-lg">
            <form method="POST" action="{{ route('asset-categories.store') }}" class="space-y-5">
                @csrf
                ...
                <flux:field>
                    ...
                    <flux:input
                        ...
                        required
                    />
                    <flux:description>2 uppercase letters, unique. Used to generate asset codes (e.g. <strong>VE</strong>-1, <strong>AC</strong>-1).</flux:description>
                    @error('code')
                        <flux:error>{{ $message }}</flux:error>
                    @enderror
                </flux:field>
