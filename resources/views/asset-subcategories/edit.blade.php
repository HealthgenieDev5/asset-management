<x-layouts::app :title="__('Edit Subcategory')">
        <div class="mb-6 flex items-center gap-3">
            ...
            <div>
                <flux:heading size="xl" class="font-extrabold">
                    Edit <span class="text-accent">Subcategory</span>
                </flux:heading>
                <flux:text class="mt-0.5 text-zinc-400">Update subcategory details.</flux:text>
            </div>
        </div>

        <div class="max-w-lg">
            <form method="POST" action="{{ route('asset-subcategories.update', $subcategory) }}" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <flux:field>
                    ...
                    <flux:select id="asset_category_id" name="asset_category_id" required>
                        ...
                        @foreach ($categories as $cat)
                            <flux:select.option value="{{ $cat->id }}" :selected="old('asset_category_id', $subcategory->asset_category_id) == $cat->id">
                                {{ $cat->code }} — {{ $cat->name }}
                            </flux:select.option>
                        @endforeach
                    </flux:select>
                ...
                <flux:field>
                    ...
                    <flux:select id="status" name="status">
                        <flux:select.option value="active" :selected="old('status', $subcategory->status) === 'active'">Active</flux:select.option>
                        <flux:select.option value="inactive" :selected="old('status', $subcategory->status) === 'inactive'">Inactive</flux:select.option>
                    </flux:select>
                    @error('status')
                        <flux:error>{{ $message }}</flux:error>
                    @enderror
