<x-layouts::app :title="__('Asset Subcategories')">
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <flux:text class="mt-1 text-zinc-500 dark:text-zinc-400">Manage asset subcategories grouped under categories.</flux:text>
            </div>
        ...
        {{-- Filters --}}
        @php
            $activeFilters = array_filter(request()->only(['search', 'category_id', 'status']));
            ...
            <div class="rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                ...
                <div class="flex items-center justify-between border-b border-zinc-100 px-4 py-2.5 dark:border-zinc-800">
                    ...
                    <div class="flex items-center gap-1.5">
                        @if ($hasFilters)
                            <a href="{{ route('asset-subcategories.index') }}"
                    ...
                    @forelse ($subcategories as $sub)
                        <tr class="hover:bg-accent/5 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                ...
                                <span class="ml-1.5 text-zinc-500 dark:text-zinc-400">{{ $sub->category->name ?? '—' }}</span>
                            </td>
                            <td class="px-4 py-3 font-medium text-zinc-900 dark:text-zinc-100">{{ $sub->name }}</td>
                            <td class="px-4 py-3 text-zinc-500 dark:text-zinc-400">{{ $sub->code ?: '—' }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    <form method="POST" action="{{ route('asset-subcategories.destroy', $sub) }}" onsubmit="confirmDelete(this, 'Delete this subcategory?'); return false;">
                                        @csrf
                                        @method('DELETE')
