<x-layouts::app :title="__('Reminders')">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <flux:text class="mt-1 text-zinc-400">Upcoming and overdue expiry dates across all assets.</flux:text>
        </div>
    </div>
    ...
    <div class="mb-5 flex gap-2 border-b border-zinc-800 pb-0">
        @foreach ($filters as $key => $label)
            <a href="{{ request()->fullUrlWithQuery(['filter' => $key]) }}"
               class="px-4 py-2 text-sm font-medium transition-colors border-b-2 -mb-px
                ...
                {{ $label }}
                @if (isset($counts[$key]))
                    <span class="ml-1.5 rounded-full bg-zinc-800 px-1.5 py-0.5 text-xs">{{ $counts[$key] }}</span>
                @endif
            </a>
        @endforeach
    </div>
    ...
    @if ($reminders->isEmpty())
        <div class="flex items-center justify-center rounded-xl border border-dashed border-zinc-700 bg-zinc-900 py-20 text-center">
            <div>
                <flux:icon.bell-alert class="mx-auto size-12 text-zinc-600" />
                <flux:heading class="mt-4 text-zinc-400">No Reminders Found</flux:heading>
                        ...
                        <tr class="hover:bg-zinc-800/30 transition-colors">
                            <td class="px-4 py-3">
                                <a href="{{ route('assets.show', $reminder['asset']) }}"
                                   class="group flex items-center gap-2">
                                    ...
                                    <span class="truncate text-zinc-300 group-hover:text-zinc-100 max-w-40">{{ $reminder['asset']->asset_name }}</span>
                                </a>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-1.5">
                                    <flux:icon :icon="$icon" class="size-4 shrink-0 text-zinc-400" />
                                    <span class="text-zinc-300">{{ $reminder['type'] }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-zinc-400 max-w-45 truncate">
                                {{ $reminder['detail'] ?: '—' }}
                            </td>
                            <td class="px-4 py-3 text-zinc-200 whitespace-nowrap">
                            ...
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('assets.show', [$reminder['asset'], 'tab' => $reminder['tab']]) }}"
                                   class="text-xs text-zinc-400 hover:text-accent transition-colors">
                                    View →
                                </a>
                            </td>
                        </tr>
